...
public class ColorRatingBar extends View {
    ...
    private void parseAttrs(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.ColorRatingBar);

        borderColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_borderColor, getResources().getColor(R.color.common_color_orange_light));
        fillColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_fillColor, borderColor);
        starBackgroundColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_starBackgroundColor, Color.TRANSPARENT);
        backgroundColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_backgroundColor, Color.TRANSPARENT);

        pressedBorderColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_pressedBorderColor, borderColor);
        pressedFillColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_pressedFillColor, fillColor);
        pressedStarBackgroundColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_pressedStarBackgroundColor, starBackgroundColor);
        pressedBackgroundColor = typedArray.getColor(R.styleable.ColorRatingBar_srb_pressedBackgroundColor, backgroundColor);

        numberOfStars = typedArray.getInteger(R.styleable.ColorRatingBar_srb_numberOfStars, 5);

        starsSeparation = typedArray.getDimensionPixelSize(R.styleable.ColorRatingBar_srb_starsSeparation, (int) valueToPixels(4, Dimension.DP));
        maxStarSize = typedArray.getDimensionPixelSize(R.styleable.ColorRatingBar_srb_maxStarSize, Integer.MAX_VALUE);
        desiredStarSize = typedArray.getDimensionPixelSize(R.styleable.ColorRatingBar_srb_starSize, Integer.MAX_VALUE);
        stepSize = typedArray.getFloat(R.styleable.ColorRatingBar_srb_stepSize, 0.1f);
        starBorderWidth = typedArray.getFloat(R.styleable.ColorRatingBar_srb_starBorderWidth, 5f);
        starCornerRadius = typedArray.getFloat(R.styleable.ColorRatingBar_srb_starCornerRadius, 6f);

        rating = normalizeRating(typedArray.getFloat(R.styleable.ColorRatingBar_srb_rating, 0f));
        isIndicator = typedArray.getBoolean(R.styleable.ColorRatingBar_srb_isIndicator, false);
        drawBorderEnabled = typedArray.getBoolean(R.styleable.ColorRatingBar_srb_drawBorderEnabled, true);
        gravity = Gravity.fromId(typedArray.getInt(R.styleable.ColorRatingBar_srb_gravity, Gravity.Left.id));
    ...
    private void validateAttrs() {
        ...
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize > maxStarSize) {
    ...
    private void animateRating(AnimationBuilder builder) {
        ...
        ratingAnimator.addListener(new AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animator) {
                if (ratingListener != null) {
                    ratingListener.onRatingChanged(ColorRatingBar.this, rating, false);
                }
            }
    ...
    public interface OnRatingBarChangeListener {
         ...
         */
        void onRatingChanged(ColorRatingBar simpleRatingBar, float rating, boolean fromUser);
    ...
    public class AnimationBuilder {
        ...
        private AnimationBuilder(ColorRatingBar ratingBar) {
