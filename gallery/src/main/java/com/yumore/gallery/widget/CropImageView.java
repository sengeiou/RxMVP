package com.yumore.gallery.widget;
...
public class CropImageView extends AppCompatImageView {
    ...
    public CropImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMaskColor = typedArray.getColor(R.styleable.CropImageView_cropMaskColor, mMaskColor);
        mBorderColor = typedArray.getColor(R.styleable.CropImageView_cropBorderColor, mBorderColor);
        mBorderWidth = typedArray.getDimensionPixelSize(R.styleable.CropImageView_cropBorderWidth, mBorderWidth);
        mFocusWidth = typedArray.getDimensionPixelSize(R.styleable.CropImageView_cropFocusWidth, mFocusWidth);
        mFocusHeight = typedArray.getDimensionPixelSize(R.styleable.CropImageView_cropFocusHeight, mFocusHeight);
        mDefaultStyleIndex = typedArray.getInteger(R.styleable.CropImageView_cropStyle, mDefaultStyleIndex);
    ...
    private void initImage() {
        ...
        if (!isInited || drawable == null) {
            return;
        }

        mode = NONE;
        matrix = getImageMatrix();
        mImageWidth = mRotatedImageWidth = drawable.getIntrinsicWidth();
        mImageHeight = mRotatedImageHeight = drawable.getIntrinsicHeight();
        //计算出焦点框的中点的坐标和上、下、左、右边的x或y的值
        int viewWidth = getWidth();
        int viewHeight = getHeight();
        float midPointX = viewWidth >> 1;
        float midPointY = viewHeight >> 1;
        ...
        //图像中点为中心进行缩放
        matrix.setScale(scale, scale, mImageWidth >> 1, mImageHeight >> 1);
...
//                    bitmap.recycle();
    ...
    private static class InnerHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            File saveFile = (File) message.obj;
            switch (message.what) {
                case SAVE_SUCCESS:
                    if (mListener != null) {
