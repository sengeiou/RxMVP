...
public final class LandmarkEngine {
    ...
    private final SparseArray<com.yumore.landmark.OneFace> mFaceArrays;
    // 手机当前的方向，0表示正屏幕，3表示倒过来，1表示左屏幕，2表示右屏幕
    private float mOrientation;
    private boolean mNeedFlip;

    private LandmarkEngine() {
        mFaceArrays = new SparseArray<com.yumore.landmark.OneFace>();
    }
     ...
     * @return
     */
    public com.yumore.landmark.OneFace getOneFace(int index) {
        com.yumore.landmark.OneFace oneFace = null;
        synchronized (mSyncFence) {
            oneFace = mFaceArrays.get(index);
            if (oneFace == null) {
                oneFace = new com.yumore.landmark.OneFace();
            }
        }
        return oneFace;
    }
     ...
     * @param index
     */
    public void putOneFace(int index, com.yumore.landmark.OneFace oneFace) {
        synchronized (mSyncFence) {
     ...
     * @return
     */
    public SparseArray<com.yumore.landmark.OneFace> getFaceArrays() {
        return mFaceArrays;
    }
    ...
    public void calculateExtraFacePoints(float[] vertexPoints, int index) {
        ...
        com.yumore.landmark.OneFace oneFace = mFaceArrays.get(index);
        // 复制关键点的数据
        System.arraycopy(oneFace.vertexPoints, 0, vertexPoints, 0, oneFace.vertexPoints.length);
        // 新增的人脸关键点
        float[] point = new float[2];
        // 嘴唇中心
        com.yumore.landmark.FacePointsUtils.getCenter(point,
