package com.yumore.filter.widget;
...
public class GLImageSurfaceView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    // 输入纹理
    protected int mInputTexture = com.yumore.filter.glfilter.utils.OpenGLUtils.GL_NOT_TEXTURE;
    // 图片输入滤镜
    ...
    public GLImageSurfaceView(Context context, AttributeSet attrs) {
        ...
        mVertexBuffer = com.yumore.filter.glfilter.utils.OpenGLUtils.createFloatBuffer(TextureRotationUtils.CubeVertices);
        mTextureBuffer = com.yumore.filter.glfilter.utils.OpenGLUtils.createFloatBuffer(TextureRotationUtils.TextureVertices);
    }
    ...
    public void onPause() {
        super.onPause();
        mInputTexture = com.yumore.filter.glfilter.utils.OpenGLUtils.GL_NOT_TEXTURE;
        mColorFilter = null;
        mDisplayFilter = null;
        mInputFilter = null;
    }
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        if (mInputTexture == com.yumore.filter.glfilter.utils.OpenGLUtils.GL_NOT_TEXTURE) {
            mInputTexture = com.yumore.filter.glfilter.utils.OpenGLUtils.createTexture(mBitmap, mInputTexture);
        }
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (takePicture) {
            ...
            com.yumore.filter.glfilter.utils.OpenGLUtils.checkGlError("glReadPixels");
            buf.rewind();
            takePicture = false;
            if (mCaptureCallback != null) {
    ...
    private void createColorFilter(ResourceData resourceData) {
        ...
        try {
            ...
            mColorFilter = new com.yumore.filter.glfilter.color.GLImageDynamicColorFilter(getContext(), dynamicColor);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
